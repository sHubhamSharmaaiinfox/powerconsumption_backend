...
class getMembership(APIView):
    def post(self, request):
        ...
        membership=PackagesSerial(membership).data
        return Response(
            {"status":True,
            "message":"success",
            "data":membership},
...
class createqrcode(APIView):
    def post(self, request):
        ...
        if True:
            plan_amount=Packages.objects.get(id=id_).amount
            print(plan_amount)
...
class Paymentreceived(APIView):
    def post(self, request):
        ...
        plan= Packages.objects.get(id=plan_id)
        comments= data.get('comment')
        image_path = data.get('imagepath') 
        start_date_obj = datetime.now()
        expiry_date = start_date_obj + timedelta(days=int(plan.plan_period) * 30)
        AdminMembership.objects.create(user_id=usr,plan_id=plan,status="0",amount=plan.amount,expire_date=expiry_date)
...
class GetAllUser(APIView):
    def get(self,request):
        ...
        try:
            ...
            return Response({'status': False, 'message': 'Unauthorized'}, status=status.HTTP_401_UNAUTHORIZED)
        user_id = [i.child_id.id for i in UserLinked.objects.filter(parent_id=usr.id)]
        user = User.objects.filter(id__in = user_id,role="user",verified_at=True)
...
class VerifiedUsers(APIView):
    def get(self, request):
        ...
        user_id  = [i.child_id.id for i in UserLinked.objects.filter(parent_id=usr.id)]
        verified_users = User.objects.filter(status="1",role="user",id__in = user_id)
...
class UnverifiedUsers(APIView):
    def get(self, request):
        ...
        user_id  = [i.child_id.id for i in UserLinked.objects.filter(parent_id=usr.id)]
        unverified_users = User.objects.filter(status="0",role="user",id__in=user_id)
        # Serialize user data
...
class MetersData(APIView):
    def post(self,request):
        ...
        meters = [{"username":i.member_id.user_id.username,"created_at":i.created_at,"location":i.location,"status":i.status,"name":i.name,"token":i.token,"id":i.id} for i in UserMeters.objects.filter(member_id__in = members_id)]
...
class MeterCreate(APIView):
    def post(self,request):
        ...
        id_ = data.get('id')
        plan_id = UserMemberships.objects.get(user_id=id_,status='1')
...
class GetUserCount(APIView):
    def get(self, request):
        ...
        try:
            ...
            return Response({"status": False, "message": "User not found"}, status=status.HTTP_404_NOT_FOUND)
        try:
            child_id = [i.child_id.id for i in UserLinked.objects.filter(parent_id = usr.id)]
            print("child id",child_id)
            active_user=User.objects.filter(status='1',role="user",id__in=child_id).count()
            inactive_user=User.objects.filter(status='0',role="user",id__in=child_id).count()
            total_user = User.objects.filter(role="user",id__in=child_id).count()
            total_subscription= UserMemberships.objects.filter(status="1",user_id__in=child_id).count()
            total_amount = sum([float(i.amount) for i in UserMemberships.objects.filter(user_id__in=child_id)])
            ...
            return Response({"status": False, "message": "Membership not found"}, status=status.HTTP_400_BAD_REQUEST)
...
class TotalSubscriptionChart(APIView):
    def get(self, request):
        ...
        try:
            ...
            user_data = (
                UserMemberships.objects.filter(id__in=user_id)
                .annotate(
                    created_at_datetime=Cast('date', output_field=DateTimeField())  # Ensure `date` is cast properly
                )
            ...
            user_data_dict = {
                entry['month'].replace(tzinfo=None): entry['total_amount']
                for entry in user_data if entry['month']
            }

            chart_data = [{
                "x": month.strftime('%B %Y')[:3],  # Short month name
                "y": user_data_dict.get(month.replace(tzinfo=None), 0)  # Total amount
            } for month in all_months]

            return Response({
                "status": True,
                "message": "Subscribers Amount By Month fetched",
                "data": [{"name": "amounts", "data": chart_data}]
            }, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({
                "status": False,
                "message": "Subscribers Amount by month not fetched",
                "error": str(e)
            }, status=status.HTTP_404_NOT_FOUND)
...
class UserCountByMonthAPIView(APIView):
    def get(self, request):
        ...
        try:
            ...
            user_counts = (
                User.objects.filter(role='user',id__in=user_id).annotate(
    ...
    def get(self, request):
        ...
        try:
            ...
            UserMemberships.objects.annotate(
                created_at_datetime=Cast('date', output_field=DateTimeField())  
            )
            .annotate(month=TruncMonth('created_at_datetime'))  
            .filter(created_at_datetime__year=current_year)  
            .values('month')
            ...
            chart_data = {
                ...
                "counts": [user_count_dict.get(month.replace(tzinfo=None), 0) for month in all_months], 
            }
...
class deviceCountByMonthAPIView(APIView):
    def get(self, request):
        ...
        try:
            user_id = [i.child_id.id for i in UserLinked.objects.filter(parent_id = usr.id)]
            members_id = [i.id for i in UserMemberships.objects.filter(user_id__in = user_id)]
            ...
            user_counts = (
            UserMeters.objects.filter(member_id__in = members_id).annotate(
                created_at_datetime=Cast('created_at', output_field=DateTimeField()) 
            )
            .annotate(month=TruncMonth('created_at_datetime'))  
            .filter(created_at_datetime__year=current_year)  
