...
class InactiveUsers(APIView):
    def get(self,request):
        ...
        try:
            ...
            return Response({"status": False, "message": "User not found"}, status=status.HTTP_404_NOT_FOUND)
        data = [{"id":i.child_id.id,"created_by":i.parent_id.username,"username":i.child_id.username,"email":i.child_id.email,"first_name":i.child_id.first_name,"last_name":i.child_id.last_name,"status":i.child_id.status,"date":i.date} for i in UserLinked.objects.all() if i.child_id.status=="0"]
        return Response({"status":True,"message":"success","data":data},status=status.HTTP_200_OK)
...
class DevicesByMonth(APIView):
    def get(self, request):
        ...
        try:
            ...
            return Response({"status": False, "message": "Unknown Request"}, status=status.HTTP_404_NOT_FOUND)                
...
class AdminDetail(APIView):
    def post(self, request):
        ...
        try:
            ...
            plan_name1=AdminMembership.objects.filter(user_id=id,status='1')
            print(plan_name1)
            if len(plan_name1) >0:
                plan_name=plan_name1[0].plan_id.name     
            else:
                plan_name = None
            usercount = len(UserLinked.objects.filter(parent_id=userdetail.id))
            userdetailser=UserSerial(userdetail).data
            data = {"userdata":userdetailser,"plan_name":plan_name,"user_count":usercount}
            return Response({"status":True,"message":"success","data":data},status=status.HTTP_200_OK)
        except:
